...
<body>
    <div class="container">
        ...
        <?php if (empty($images)): ?>
            <div class="empty">
                <p>Нет фотографий. <a href="upload.php">Загрузите первую</a></p>
            </div>
        <?php else: ?>
            <div class="gallery">
                <?php foreach ($images as $img): ?>
                    <div class="gallery-item" data-id="<?= $img['id'] ?>">
                        ...
                        <div class="stats">
                            <span class="views">👁️ <span class="views-count-<?= $img['id'] ?>"><?= $img['views'] ?></span></span>
                            <span class="likes" onclick="like(<?= $img['id'] ?>, this)">❤️ <span class="likes-count-<?= $img['id'] ?>"><?= $img['likes'] ?></span></span>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <script>
        function openModal(src, id, imgElement) {
            ...
            fetch('update.php?action=view&id=' + id)
                ...
                .then(data => {
                    if (data.success) {
                        document.querySelector('.views-count-' + id).innerText = data.views;
                    }
                });
        }
